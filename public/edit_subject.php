...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        ...
        $query .= "position = {$position}, ";   // Don't forget space after every command
        $query .= "visible = {$visible} ";
        $query .= "WHERE id = {$id} ";
        $query .= "LIMIT 1";
        $result = mysqli_query($connection, $query);

        // check if result was successful and also the number of affected rows (for UPDATE and DELETE affected rows needs to be checked)
...
<div id="main">
  <div id="navigation">
      ...
      <!-- Shows all menus in subjects and pages in each subject
            Also bolds the currently selected link -->
